...
batchsize=1
curr_lr = 1e-4
reinit_point=5 # the no. of epoch when we decay the lr and reinitialize the noise encoder
...
testlist = [1,2,6,100001,100002,100003,100010]#test both training and testing images
...
for ind in range(reinit_point+11):
    ...
    for idx in testlist:
        ...
        middle=np.concatenate((output[3,:,:,:],output[4,:,:,:],output[5,:,:,:]),axis=1)
        bottom=np.concatenate((output[6,:,:,:],output[7,:,:,:],output[8,:,:,:]),axis=1)
        scipy.misc.toimage(np.concatenate((upper,middle,bottom),axis=0),cmin=0,cmax=255).save("gta_demo/%04d/%06d_output.png"%(ind,idx))
        #scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save("gta_demo/%04d/%06d_output.png"%(epoch,ind))
    if ind==reinit_point:#reinitialize the noise encoder to avoid overfitting
